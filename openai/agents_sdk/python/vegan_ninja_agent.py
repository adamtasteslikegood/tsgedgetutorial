...
class VeganMatreDSchema__Ingredients(BaseModel):
  wet: list[str]
  dry: list[str]
  other: list[str] | None = None


class VeganMatreDSchema(BaseModel):
  name: str
  description: str | None = None
  prepTime: float
  cookTime: float
  servings: float
  ingredients: VeganMatreDSchema__Ingredients
...
async def run_workflow(workflow_input: WorkflowInput):
  with trace("TastesLikeGood: Agentic Ninja Recipe Warrior  Edition"):
    state = {
    ...
    vegan_matre_d_result_temp = await Runner.run(
      vegan_matre_d,
      input=[
    ...
    conversation_history.extend([item.to_input_item() for item in vegan_matre_d_result_temp.new_items])

    vegan_matre_d_result = {
      "output_text": vegan_matre_d_result_temp.final_output.json(),
      "output_parsed": vegan_matre_d_result_temp.final_output.model_dump()
    }
